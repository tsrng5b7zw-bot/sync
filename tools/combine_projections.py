...
from one file takes the others' average. Independent models make different mistakes, so their average
...
def main():
    ...
    keys = set().union(*(rows.keys() for _, rows in loaded))
    ...
    for k in sorted(keys):
        ...
        vals = [sum(wi * float(r.get(g) or 0) for wi, r in have) / tw for g in common]
        base = have[0][1]
